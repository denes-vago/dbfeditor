...
final class DBFIOUtil {
    ...
    static MemoReadResult readMemo(byte[] data, Charset charset, String pointerText, String fieldName, int rowIndex) {
        if (data == null) {
            return new MemoReadResult(pointerText, null);
        }
        String trimmedPointer = pointerText == null ? "" : pointerText.trim();
        if (trimmedPointer.isEmpty()) {
            return new MemoReadResult("", null);
        }
        try {
            int blockNumber = Integer.parseInt(trimmedPointer);
            if (blockNumber <= 0) {
                return new MemoReadResult("", null);
            }
            long start = (long) blockNumber * DBT_BLOCK_SIZE;
            if (start >= data.length) {
                return new MemoReadResult(
                    trimmedPointer,
                    "A(z) " + fieldName + " memo mező a(z) " + (rowIndex + 1) + ". rekordban érvénytelen DBT blokkra mutat: " + blockNumber + "."
                );
            }
            ...
            return new MemoReadResult(trimRight(new String(buffer.toByteArray(), charset)).replace("\r\n", "\n"), null);
        } catch (Exception e) {
            return new MemoReadResult(
                trimmedPointer,
                "A(z) " + fieldName + " memo mező a(z) " + (rowIndex + 1) + ". rekordban nem olvasható memo mutatót tartalmaz: " + trimmedPointer + "."
            );
        }
    }

    record MemoReadResult(String value, String warning) {
    }
}
