        ...
        try (RandomAccessFile file = new RandomAccessFile(path.toFile(), "r")) {
            ...
            for (long i = 0; i < recordCount; i++) {
                ...
                for (DBFEngine.FieldDescriptor field : fields) {
                    row.add(DBFValueCodec.formatValue(field, recordBuffer, offset, charset, memoData));
                    offset += field.length();
                }
                records.add(row);
            }
            ...
            return new DBFEngine.DBFFile(version, lastUpdate, recordCount, headerLength, recordLength, fields, records);
        }
    }
